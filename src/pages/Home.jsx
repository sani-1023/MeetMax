...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    if (!Array.isArray(posts) || posts.length === 0) {
      posts = defaultPosts;
      localStorage.setItem("meetmax_posts", JSON.stringify(defaultPosts));
    }
    setPosts(posts);
  }, []);
        ...
        {posts.map((post) => (
          <div key={post.id} className="bg-white p-4">
            ...
            {post.type === "photo" && (
              <div className="mb-3">
                <img
                  src={post.imageUrl || DummyImage}
                  alt="Post content"
                  className="w-full h-48 max-h-60 rounded-lg object-contain bg-gray-100"
                />
              </div>
            )}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex -space-x-1">
                    ...
                    {event.totalAttendees > event.attendees.length && (
                      <div className="w-6 h-6 rounded-full bg-gray-500 text-white border-2 border-white flex items-center justify-center text-xs">
                        +{event.totalAttendees - event.attendees.length}
                      </div>
                    )}
                  </div>
